...
- 사용자가 잘못된 값을 입력할 경우 `IllegalArgumentException`를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 입력을 다시 받는다.
...
- [X] 게임 시작
- [X] 자동차 이름 입력
  - [X] [ERROR] 쉼표를 구분자로 나눴을 때, 5 글자 이하여야 한다.
  - [x] [ERROR] 동일한 이름은 중복 체크 해야 한다.
- [X] 자동차 상태 저장
  - [X] 입력받은 이름에 따라 자동차 생성
  - [X] 전체 자동차 이동
  - [X] 우승자 조회
- [X] 몇 번 이동할 것인지 입력
  - [X] [ERROR] 입력 값이 숫자여야 한다.
- [X] 각 자동차들 이동
- [X] 각 자동차들 이동 후 출력
- [ ] 우승자 출력